...
import { 
  Plus, Trash2, ShieldAlert, Clock, Save, FileText, CheckCircle2, Loader2, ArrowLeft 
} from "lucide-react";
...
const EditExam = () => {
  ...
  useEffect(() => {
    const fetchExam = async () => {
      try {
        ...
        setExamData({
            ...data.exam,
            allowedAttempts: data.exam.allowedAttempts || 1
        });
      } catch (err) {
  ...
  // ✅ FIXED: Handle basic inputs (Enforces Numbers for number inputs)
  ...
  // 2. Final Submission Logic (PUT Request)
  const handleSubmit = async (publishStatus) => {
    ...
    try {
      ...
      toast.success(publishStatus ? "Exam Published Successfully!" : "Exam Changes Saved");
      navigate("/admin/dashboard");
    } catch (err) {
      ...
      <div className="max-w-5xl mx-auto pb-24">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
          <div>
            ...
            <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Edit Examination</h1>
            <p className="text-slate-500 mt-1">Update questions, duration, or security settings.</p>
          </div>
                  ...
                  <div className="flex flex-col md:flex-row gap-6 pt-6 border-t border-slate-50">
                    <div className="flex-1 space-y-2">
                      ...
                      <select 
                        ...
                        className="w-full bg-slate-50 p-4 rounded-2xl outline-none border border-transparent focus:border-indigo-100 font-bold text-slate-700 cursor-pointer"
                      >
