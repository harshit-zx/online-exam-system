...
const AdminDashboard = () => {
  ...
  const handleTogglePublish = async (exam) => {
    try {
      ...
      setExams(
        exams.map((e) =>
          e._id === exam._id ? { ...e, isPublished: updatedStatus } : e,
        ),
      );
      toast.success(
        updatedStatus ? "Exam is now Live!" : "Exam moved to Drafts",
      );
    } catch (err) {
  ...
  const handleDeleteExam = async (id) => {
    if (
      window.confirm(
        "Are you sure? This will permanently delete the exam and all associated student results.",
      )
    ) {
      try {
  ...
  const handleDeleteResult = async (id) => {
    if (
      window.confirm(
        "Delete this result? This will clear the attempt and allow the student to retake the exam if they hit their limit.",
      )
    ) {
      try {
      ...
      <div className="min-h-screen bg-[#f8fafc] flex selection:bg-indigo-100 selection:text-indigo-900 font-sans">
        ...
        <aside className="w-64 bg-slate-900 hidden lg:flex flex-col text-white p-6 sticky top-0 h-screen shadow-2xl">
          <div className="flex items-center gap-3 mb-12 px-2">
            ...
            <span className="font-black text-2xl tracking-tight text-transparent bg-clip-text bg-linear-to-r from-white to-slate-400">
              AdminPro
            </span>
          </div>
          ...
          <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
            <div>
              ...
              <p className="text-slate-500 mt-2 font-medium text-lg">
                Manage examinations and monitor student performance.
              </p>
            </div>
            ...
            <div className="flex items-center gap-4 w-full md:w-auto">
              <div className="hidden md:flex bg-white p-2 rounded-2xl shadow-sm border border-slate-200 items-center gap-3 px-4 mr-2">
                <div className="w-10 h-10 bg-linear-to-tr from-indigo-600 to-blue-500 text-white rounded-full flex items-center justify-center font-bold text-lg shadow-inner">
            ...
            exams.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 items-stretch">
                {exams.map((exam) => (
                  <div
                    key={exam._id}
                    className="group bg-white border border-slate-200 p-6 rounded-4xl hover:-translate-y-1 hover:shadow-xl hover:shadow-indigo-500/10 hover:border-indigo-200 transition-all duration-300 flex flex-col h-full"
                  >
                    <div className="flex justify-between items-start mb-5">
                      <span
                        className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 shadow-sm border ${exam.isPublished ? "bg-emerald-50 text-emerald-700 border-emerald-100" : "bg-slate-50 text-slate-500 border-slate-200"}`}
                      >
                        {exam.isPublished ? (
                          <>
                    ...
                    <div className="flex items-center gap-3 text-slate-600 text-sm font-bold mb-6">
                      <div className="flex items-center gap-2 bg-slate-50 border border-slate-100 px-3 py-2 rounded-xl shadow-sm">
                        ...
                        <div className="p-3 bg-white border-t border-slate-100 space-y-2 text-xs font-bold">
                          ...
                          <div className="flex justify-between items-center p-2.5 rounded-xl bg-slate-50">
                            <span className="text-slate-600 flex items-center gap-1.5">
                              <Repeat size={14} /> Attempts Allowed
                            </span>
                            <span className="font-black text-slate-900 bg-white px-2 py-0.5 rounded shadow-sm border border-slate-100">
              ...
              <div className="bg-white rounded-[2.5rem] p-16 md:p-24 text-center border border-dashed border-slate-300 shadow-sm">
                ...
                <p className="text-slate-500 font-medium max-w-sm mx-auto mb-8">
                  You haven't created any examinations yet. Click the button
                  above to get started.
                </p>
                <Link
                  to="/admin/createexam"
                  className="inline-flex items-center justify-center px-6 py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-600/20"
                >
          ...
          results.length > 0 ? (
                ...
                <table className="min-w-full divide-y divide-slate-100">
                  <thead className="bg-slate-50">
                    <tr>
                      ...
                      <th className="px-6 py-5 text-center text-xs font-black text-slate-500 uppercase tracking-widest">
                        Actions
                      </th>
                    </tr>
                  </thead>
                        ...
                        >
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center gap-3">
                              <span
                                className={`text-sm font-black px-3 py-1.5 rounded-lg border ${isPassing ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-red-50 text-red-600 border-red-100"}`}
                              >
            ...
            <div className="bg-white rounded-[2.5rem] p-16 md:p-24 text-center border border-dashed border-slate-300 shadow-sm">
              ...
              <p className="text-slate-500 font-medium max-w-sm mx-auto">
                Once students start taking your exams, their automated
