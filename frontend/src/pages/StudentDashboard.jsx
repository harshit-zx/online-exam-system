...
import { BookOpen, FileText, Layout, LogOut, Loader2, AlertCircle, Award } from 'lucide-react';
...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        setError("Could not load dashboard data. Please ensure you are logged in.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const getAttemptsInfo = (examId, allowed) => {
    const attemptsTaken = recentResults.filter(r => r.exam?._id === examId).length;
      ...
      <main className="flex-1 p-4 md:p-6 lg:p-10 overflow-y-auto">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-4">
          <div>
            ...
            <p className="text-slate-500 mt-1 font-medium">Ready to excel in your assessments today?</p>
          </div>
          <div className="bg-white p-2 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-3 px-4 w-max">
            ...
            <div className="text-sm pr-2">
              ...
              <p className="text-xs text-slate-400 font-bold uppercase tracking-wider mt-0.5">Student Account</p>
            </div>
          </div>
        </header>
        ...
        <section>
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 bg-white rounded-[2.5rem] border border-slate-100 shadow-sm">
              ...
              <p className="text-slate-500 font-bold tracking-wide">Fetching your dashboard...</p>
            </div>
          ) : error ? (
            <div className="bg-red-50 border border-red-100 p-8 rounded-[2.5rem] flex flex-col items-center text-center shadow-sm">
              ...
              <h3 className="text-red-900 font-black text-xl mb-2">{error}</h3>
              <button onClick={() => window.location.reload()} className="mt-4 px-8 py-3 bg-red-600 text-white rounded-xl font-bold hover:bg-red-700 transition-all shadow-lg shadow-red-200 active:scale-95">
                Retry Connection
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 items-stretch">
              {availableExams.map((exam) => {
                const { remaining } = getAttemptsInfo(exam._id, exam.allowedAttempts || 1);
                const isLocked = remaining <= 0;

                return (
                  // ✅ FIX 1: Added h-full and flex flex-col to prevent grid squashing/overlapping
                  <div key={exam._id} className="relative h-full flex flex-col transition-transform hover:-translate-y-1 duration-300">
                    <ExamCard 
                      exam={exam} 
                      onClick={isLocked ? () => toast.error("No attempts remaining") : handleExamClick} 
                    ...
                    <div className="absolute top-5 left-1/2 -translate-x-1/2 z-10 pointer-events-none">
                      <span className={`text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-lg border backdrop-blur-md shadow-sm ${
                        isLocked 
            ...
            <div className="bg-white rounded-[2.5rem] p-10 sm:p-20 text-center border border-dashed border-slate-200 shadow-sm">
              ...
              <h3 className="text-2xl font-black text-slate-800 mb-2">No Exams Assigned</h3>
              <p className="text-slate-500 max-w-sm mx-auto font-medium">You don't have any pending assessments right now. Check back later!</p>
            </div>
          )}
        </section>
        ...
        <section className="mt-12 bg-white rounded-[2.5rem] p-6 md:p-8 border border-slate-100 shadow-sm">
          ...
          {loading ? (
                   ...
                   return (
                     ...
                     >
                        <div className="flex items-center gap-4 sm:gap-5">
                           <div className={`w-12 h-12 shrink-0 flex items-center justify-center rounded-xl shadow-sm ${isPassing ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-600'}`}>
                              <Award size={24} />
                           </div>
                           <div>
                              ...
                              <p className="text-xs font-bold text-slate-400 mt-1 uppercase tracking-wider">
                                {new Date(result.createdAt).toLocaleDateString(undefined, { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' })}
                              </p>
                           </div>
                        </div>
                        <div className="text-left sm:text-right shrink-0 bg-white px-5 py-3 rounded-xl border border-slate-100 shadow-sm w-full sm:w-auto flex flex-row sm:flex-col justify-between sm:justify-center items-center sm:items-end">
                           <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest sm:hidden">Final Score</p>
                           <div>
                             <p className={`text-xl font-black leading-none ${isPassing ? 'text-emerald-600' : 'text-red-600'}`}>
                                {result.score} / {result.totalMarks}
                             </p>
                             <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1 hidden sm:block">Final Score</p>
                           </div>
                        </div>
                     </div>
                   );
             ...
             <div className="text-center py-12">
               ...
               <p className="text-slate-500 font-bold text-lg mb-1">No Completed Exams</p>
               <p className="text-slate-400 text-sm font-medium max-w-xs mx-auto">Your automatic evaluation results will appear here after submission.</p>
             </div>
          )}
        </section>
      </main>
